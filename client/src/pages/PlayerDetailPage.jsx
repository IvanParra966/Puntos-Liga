...
const getTournamentGroups = (matches = [], history = []) => {
...
function StatCard({ label, value, subtitle, wide = false }) {
  return (
    <div
      className={`rounded-2xl border border-slate-800 bg-slate-950/60 p-5 shadow-[0_10px_30px_rgba(0,0,0,0.18)] ${wide ? 'sm:col-span-2' : ''
        }`}
    >
      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-fuchsia-400">
        {label}
      </p>
      <p className="mt-4 break-words text-2xl font-black text-white sm:text-3xl">
        {value}
      </p>
...
function TournamentHistoryCard({ group }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/40">
      <div className="flex flex-col lg:flex-row">
        <div className="w-full border-b border-slate-800 p-4 lg:w-[250px] lg:border-b-0 lg:border-r">
          <div className="flex gap-4">
            <div className="flex h-20 w-16 shrink-0 items-center justify-center rounded-xl border border-slate-700 bg-[linear-gradient(180deg,rgba(30,41,59,0.9),rgba(2,6,23,0.95))] text-sm font-black text-cyan-300">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                {group.placing ? (
                  <span className="inline-flex rounded-full bg-fuchsia-500 px-3 py-1 text-xs font-bold text-white">
                    #{group.placing}
                  </span>
                ) : null}
        ...
        <div className="min-w-0 flex-1 p-4">
          <div className="hidden grid-cols-[80px_120px_minmax(0,1fr)] gap-4 border-b border-slate-800 px-3 pb-3 text-sm font-semibold text-white md:grid">
            <p>Ronda</p>
            <p>Resultado</p>
            <p>Oponente</p>
          </div>
          ...
          <div className="mt-2 space-y-2 md:mt-0">
            {group.rounds.map((round) => (
              <div
                key={round.id}
                className="grid gap-3 rounded-xl border border-slate-800/70 bg-slate-950/40 px-3 py-3 md:grid-cols-[80px_120px_minmax(0,1fr)] md:items-center md:rounded-none md:border-0 md:border-b md:border-slate-800 md:bg-transparent md:px-3"
              >
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500 md:hidden">
                    Ronda
                  </p>
                  <p className="text-xl font-bold text-white md:text-base">
                    {round.round || '-'}
                  </p>
                </div>
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-base font-bold text-white md:text-sm">
                    {round.opponentPlayerId ? (
                      <Link
                        to={`/jugador/${round.opponentPlayerId}`}
                        className="truncate text-base font-bold text-white transition hover:text-cyan-300 md:text-sm"
                      >
                        {round.opponentName}
                      </Link>
                    ) : (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4">
      <div className="w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-950 p-6">
        ...
        <button
          type="button"
          onClick={onClose}
          className="mt-6 inline-flex rounded-full border border-cyan-400/20 bg-cyan-400/10 px-4 py-2 text-sm font-semibold text-cyan-300"
        >
          Cerrar
        </button>
      </div>
...
export default function PlayerDetailPage() {
  ...
  const tournamentGroups = useMemo(() => {
    return getTournamentGroups(matches, historicalHistory);
  }, [matches, historicalHistory]);
  ...
  return (
    <>
      <section className="space-y-8">
            ...
            <div>
              ...
              <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-950/50 p-5">
                ...
                <button
                  ...
                  className="mt-5 inline-flex items-center gap-2 rounded-full border border-fuchsia-400/20 bg-fuchsia-400/10 px-4 py-2 text-sm font-semibold text-fuchsia-300 transition hover:border-fuchsia-400/40 hover:text-white"
                >
                  <FiList size={16} />
                  Listado
                </button>
        ...
        <div className="rounded-[28px] border border-slate-800 bg-[radial-gradient(circle_at_top_left,_rgba(168,85,247,0.08),_transparent_28%),linear-gradient(180deg,rgba(2,6,23,0.96),rgba(2,6,23,0.98))] p-5 sm:p-6">
          ...
          <div className="mt-6 space-y-6">
            {!tournamentGroups.length ? (
              ...
              tournamentGroups.map((group) => (
                <TournamentHistoryCard key={group.tournamentId} group={group} />
              ))
            )}
          </div>
        </div>
      ...
      <PlaceholderModal open={listadoOpen} onClose={() => setListadoOpen(false)} />
    </>
  );
}